...
- (instancetype)initWithFrame:(NSRect)frameRect shareContext:(NSOpenGLContext*)context
{
    ...
    NSOpenGLPixelFormatAttribute attribsAlt[] =
    {
        kCGLPFAAccelerated,
		kCGLPFADoubleBuffer,
        NSOpenGLPFAAllRenderers,
...
- (void)setTracksMouseEvents:(BOOL)tracksMouseEvents
{
    ...
    if ( _tracksMouseEvents ) {
        ...
        self.trackingArea = [[[NSTrackingArea alloc]
            initWithRect:[self.focusView visibleRect]
            options: NSTrackingInVisibleRect | NSTrackingMouseMoved | NSTrackingMouseEnteredAndExited | NSTrackingActiveInActiveApp | NSTrackingAssumeInside
            owner:self
            userInfo:nil
        ] autorelease];
...
//        DDLogVerbose(@"%@ indexOfWord:%d",w.originalLabel,indexOfWord);
...
// ____________________________________________________________________________________________________ guides
...
//			DDLogVerbose(@"%f %f %f %f",_colours[i*4],_colours[1+i*4],_colours[2+i*4],_colours[3+i*4]);
		}
	}
#endif
...
//    [context presentRenderbuffer:GL_RENDERBUFFER_OES];
	...
	if ( !_layout.isTweening ) {
		...
		[[self.wordClockWordManager.word objectAtIndex:visibleWordIndex] 
	...
	if ( !_layout.isTweening ) {
		...
		[[self.wordClockWordManager.word objectAtIndex:_textureRenderRotaIndex] 
	...
	if ( !_layout.isTweening ) {
		_textureRenderRotaIndex = ( _textureRenderRotaIndex + 1 ) % _numberOfWords;
//		[[self.wordClockWordManager.word objectAtIndex:_textureRenderRotaIndex] renderMipMap];
	}
#endif
}
