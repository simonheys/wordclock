...
+ (NSArray *)extractStringContainedInOutermostBraces:(NSString *)source {
    ...
    NSInteger count = 1;
    NSInteger firstBrace;
    firstBrace = [source rangeOfString:@"("].location;

    NSInteger i = 1 + firstBrace;
    char c;
...
{
    return LogicParserStringUtilCountInstancesOf(source,'(') ==
LogicParserStringUtilCountInstancesOf(source,')');
}
...
{
    return LogicParserStringUtilContains(source,'(') ||
LogicParserStringUtilContains(source,')');
}
    */

/*
void LogicParserStringUtilExtractStringContainedInOutermostBraces(char
*source,char *leftOfBraces,char *insideBraces,char *rightOfBraces)
{
    int length;
    int count = 0;
    char *characterPtr;
