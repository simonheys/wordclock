...
- (instancetype)initWithLabel:(NSString *)label tweenManager:(TweenManager *)tweenManager
{
    self = [super init];
	if ( self ) {
//		DDLogVerbose(@"initWithLabel:%@ length:%d",label,[label length]);
...
//		[self performSelectorOnMainThread:@selector(updateColour) withObject:nil waitUntilDone:NO];// updateColour];
		[self updateColour];	
	...
	if ( value != _highlighted || !animated ) {
		...
		if ( _highlighted ) {		            
            if ( animated ) {
                ...
                [self.tweenManager tweenWithTarget:self
                    keyPath:@"colourComponentRed" 
                    toFloatValue:red 
                    delay:0.0f 
                    duration:0.15f 
...
- (void)renderInCurrentGraphicsContentAtPoint:(NSPoint)point
{
	...
	NSDictionary *fontAttributes = [NSDictionary dictionaryWithObject:_font forKey:NSFontAttributeName];
...
//	_size = NSSizeFromCGSize(CGSizeMake(_scale * (_unscaledSize.width-fontBounds.origin.x), _scale * _unscaledSize.height));
...
//	while ( width*height > _texturePixelsMaximum || ( width > 1024 || height > 1024 ) ) {
...
//	DDLogVerbose(@"Rendering with dimensions %dx%d scale:%f word:%@ length:%d",width,height,_scale,_label,[_label length]);
...
/*	
    ...
    NSRect trim = [self usedRectForSpriteData:spriteData width:width heignt:height];
    DDLogVerbose(@"width:%d height:%d trim:%@",width,height,NSStringFromRect(trim));
    */
	/*
	CGContextSetRGBStrokeColor(spriteContext, 1.0f, 1.0f, 1.0f, 1.0f);
...
//	UIGraphicsPushContext(spriteContext);
	...
	NSDictionary *fontAttributesNew = @{NSFontAttributeName: _font,
    ...
    /*
    NSRect trim2 = [self usedRectForSpriteData:spriteData width:width heignt:height];
	CGContextSetRGBStrokeColor(spriteContext, 1.0f, 1.0f, 1.0f, 1.0f);
...
//    glTexImage2D(GL_TEXTURE_2D, mipmapLevel, GL_RGBA, width, height, 0, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV, spriteData);
#endif
